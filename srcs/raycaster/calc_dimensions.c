...
/*   Updated: 2021/05/05 14:10:06 by jnivala          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../headers/doom_nukem.h"

static void	calc_drawbox(t_plgn *box, t_player *plr, t_point *x0, t_point *x1)
{
...
void	calc_dimensions(t_frame *frame, t_player *plr, t_home *home)
{
	...
	if (frame->left.wall->idx >= 0)
	{
		temp = get_portal_by_idx(frame->idx, home->sectors[frame->left.wall->idx]);
	...
	if (frame->draw_top || frame->draw_bottom)
		calc_drawbox(&frame->inner_box, plr, temp, temp->next);
	calc_drawbox(&frame->outer_box, plr, frame->left.wall, frame->left.wall->next);
