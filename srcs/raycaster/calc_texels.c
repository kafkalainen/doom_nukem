...
/*   Updated: 2021/03/17 13:43:42 by jnivala          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		calc_offsets(t_frame *frame)
{
		...
		&& frame->left.wall->x0.y == frame->left.l_pt.y)
	{
		...
		frame->uv_top_right = inv_z((t_xyz){frame->ratio * frame->tex_mult, 0.0f, frame->top_right.z});
		frame->uv_bottom_right = inv_z((t_xyz){frame->ratio * frame->tex_mult, 1.0f, frame->bottom_right.z});
	}
...
void			calc_texels(t_frame *frame, t_texture *tex)
{
	...
	calc_offsets(frame);
	frame->uv_step.x = interpolate_points(frame->uv_top_left.x, frame->uv_top_right.x, frame->top_left.x, frame->top_right.x);
	frame->uv_step.y = interpolate_points(frame->uv_top_left.y, frame->uv_bottom_left.y, frame->top_left.y, frame->bottom_left.y);
	frame->uv_step.z = interpolate_points(frame->uv_top_left.z, frame->uv_top_right.z, frame->top_left.x, frame->top_right.x);
}
