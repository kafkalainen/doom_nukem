...
/*   Updated: 2021/03/11 13:25:14 by jnivala          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calc_distances(t_frame *frame, t_texture *tex, t_player *plr)
{
	...
	frame->bottom_left.y = plr->pitch + SCREEN_HEIGHT / frame->top_left.z * 20;
	frame->bottom_right.y = plr->pitch + SCREEN_HEIGHT / frame->top_right.z * 20;
	// frame->wall_x1 = SCREEN_WIDTH - ((SCREEN_HEIGHT /
	// 	frame->l_perp_dist) * frame->left.l_pt.x) + 15;
	// frame->wall_x2 = SCREEN_WIDTH - ((SCREEN_HEIGHT /
	// 	frame->r_perp_dist) * frame->left.r_pt.x) + 15;
	...
	frame->screen_wall_len = frame->top_left.x - frame->top_right.x;
