...
void					check_ui_events(int x, int y, t_button ***blist,
							t_action *action);
int						clicked_inside_grid(int x, int y, int height,
							int width);
int						check_plr_start_and_end_sector_exists(t_editor_sector
							**list, t_plr_pos plr, int end_sector);
void					assign_end_sector(t_editor_sector **list, t_xy *click,
							t_plr_pos *end_sector, int *sector_assigned);
int						assign_player_start(
							t_editor_sector **list, t_xy *click,
...
void					link_maps(t_action *action,
							unsigned char **linked_mapname, char **map_names);
...
void					add_point_end(t_editor_walls **point,
							t_editor_walls *new);
int						add_point_to_list(t_editor *editor,
...
*/
void					close_editor_wall_list(t_editor_walls **head);
void					remove_last_point(t_editor_walls **walls,
							int *nb_of_walls);
t_bool					check_for_last_point(t_editor_sector *sector,
							t_action *action);
t_bool					check_for_intersecting_lines(t_editor_sector *sector,
							t_screen_xy p0, t_screen_xy p1);
t_bool					check_all_sectors_for_intersecting_lines(
							t_editor_sector **head,
							t_screen_xy p0, t_screen_xy p1);
t_bool					check_if_another_sector_is_inside(
							t_editor_sector *tested, t_editor_sector **head);
int						check_if_non_convex(t_editor_sector *sector);
t_bool					check_if_completely_inside(t_editor_sector *tested,
							t_editor_sector **head);
t_bool					editor_check_if_same_point(t_screen_xy p0,
							t_screen_xy p1);
...
/*		
** Sector creati		on and manipulation
*/		
...
void					read_input_string(unsigned char **string,
							t_action *action);
...
int						editor_get_sector_data(unsigned char *buf,
							...
							t_editor_sector **head);
void					reload_editor_with_defaults(t_editor *editor,
							char *path);
int						editor_parse_map_name(t_editor *editor, ssize_t size,
							unsigned char *buf, unsigned int **pos);
int						editor_get_map_header(unsigned int *pos,
							unsigned char *buf,
...
int						editor_parse_story_data(t_editor_sector *new,
							unsigned char *buf, unsigned int *pos,
							ssize_t size);
int						editor_parse_sector_data(t_editor *editor,
...
int						editor_get_entity_data(unsigned char *buf,
							t_entity_list *entity, unsigned int *pos,
							ssize_t size);
...
void					create_portal_between_sectors(t_editor_sector **head,
							t_action *action);
void					notify_user(char **str, t_buffer *buffer,
							Uint32 delta_time, int *notify_time);
void					add_notification(t_editor *editor, char *message,
							int amount);
...
void					init_static_entity(
							t_entity_list *new, t_action *action,
							t_editor_sector *sector, t_xy pos);
void					initialize_entity_data(t_entity_list *new,
							t_action *action,
							t_editor_sector *sector, t_xy pos);
...
t_bool					check_for_elevator_button(t_entity_list **head,
							int sector_idx);
t_bool					check_nbr_of_portals(t_editor_walls **walls,
							int nbr_of_walls);
...
t_bool					compare_floors_to_ceilings(t_editor_sector **sectors,
							...
							t_editor_sector *elev_sector);
t_bool					sectors_aligned(t_editor_sector *first,
							t_editor_sector *second, t_screen_xy heights,
							t_editor_sector *elev_sector);
...
void					editor_edit_change_sector_ceiling_tex(
							t_editor *editor);
void					editor_set_all_sector_ceiling_heights(
								t_editor_sector *sector,
								unsigned char **int_string);
void					editor_set_all_sector_floor_heights(
								t_editor_sector *sector,
								unsigned char **int_string);
